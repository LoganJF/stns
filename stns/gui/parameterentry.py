...
class ParameterEntryPage(tk.Frame):
    ...
    entry_params = ['Start Time (Optional)', 'End Time (Optional)', 'Minimum Threshold (Required)',
                    ...
                    'Burst Tolerance (defaults at .25s)', 'Stimulation Kind (Optional)', 'Date (Required)',
                    'Neuron (Required)', 'Stimulation (Optional)', 'Condition (Optional)', 'Notebook (Optional)']
    ...
    def __init__(self, parent, controller):
        ...
        for k,v in enumerate(self.analysis_parameters):
            ...
            elif (('defaults at' in v) and ('.25s' in v)):
                self.analysis_parameters[v].set(.25)
    ...
    def detect_spikes(self):
        """Function will call the page DataEntry to detect and then plot spikes
    ...
    def path_extrapolation(self):
        ...
        matches = ['(?<=-cond) [A-Z a-z 0-9]*',
                   'b[0-9]p[0-9]{1,3}',
                   '[0-9_]{1,3}[vV][0-9 hHzZ]{3,5}',
                   'ion[slrLR]',
    ...
    def add_more_condition_entries(self):
        """Adds more conditions for entry in the GUI
