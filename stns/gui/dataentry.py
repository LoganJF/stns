...
class DataEntry(tk.Frame):
    ...
    def add_buttons(self):
        ...
        b4 = Button(self.master, text='Train editted data', command=self.open_label_learning_window)
        b4.grid(row=1, column=3, sticky='nsew')

    def open_label_learning_window(self):
        self.time_end_fixed_data = StringVar()
        self.label_learning_window = tk.Toplevel(self.master)
        Label(self.label_learning_window, text='End Time of Fixed Data').grid(row=0, column=0, sticky='nsew')
        Entry(self.label_learning_window, textvariable=self.time_end_fixed_data).grid(row=0, column=1, sticky='nsew')
        self.alpha_param = StringVar()
        Label(self.label_learning_window, text='Alpha param').grid(row=1, column=0, sticky='nsew')
        Entry(self.label_learning_window, textvariable=self.alpha_param).grid(row=1, column=1, sticky='nsew')

        b0 = Button(self.label_learning_window, text='Label and Predict', command=self.learn_labeled_data)
        b0.grid(row=0, column=2, sticky='nsew')
        b1 = Button(self.label_learning_window, text='Undo', command=self.undo_labeling)
        b1.grid(row=1, column=2, sticky='nsew')

    def undo_labeling(self):
        self.spikes_indices = self.detected_spikes_edit
        print('Not implemented yet')
        #self.plot_detected_spikes()

    def learn_labeled_data(self):
        page = self.controller.get_page("ParameterEntryPage")
        minimum_isi = float(page.analysis_parameters['Required ISI (defaults at .01s)'].get())
        minimum_peak = float(page.analysis_parameters['Minimum Threshold (Required)'].get())
        try:
        ...
        all_spikes_indices = find_spike_indices(data_dict=self.channel_dict, minimum_isi=minimum_isi,
                                                 minimum_peak=minimum_peak, maximum_peak=maximum_peak,
                                                 zscore_data=zscore_data, find_troughs=find_troughs,
                                                 start_time=start_time, end_time=end_time)
        ...
        end_time = int(self.time_end_fixed_data.get())
        alpha_param = float(self.alpha_param.get())

        page = self.controller.get_page("SelectChannelPage")
        times = page.times
        signal = page.data

        self.detected_spikes_edit = time_of_spike[time_of_spike < end_time]
        full_spikes = all_spikes_indices
        full_spikes_time = times[full_spikes]
        sig_time = times

        labels = np.array(
            [0 if x in self.detected_spikes_edit else 1 for x in full_spikes_time[full_spikes_time < end_time]])

        win_view = sliding_window_view(signal, window_shape=30)

        nearest_t = np.array([find_nearest(sig_time, st) for st in
                              full_spikes_time[full_spikes_time < end_time]])  # detected_spikes_edit])
        X = win_view[nearest_t - 15]

        label_spread = LabelSpreading(kernel="knn", alpha=alpha_param)#0.8)#(gamma=0.25, max_iter=20)#(kernel="knn", alpha=0.8)
        label_spread.fit(X, labels)

        nearest_t = np.array([find_nearest(sig_time, st) for st in full_spikes_time])
        full_X = win_view[nearest_t - 15]
        predicted_labels = label_spread.predict(full_X)

        valid_spikes_locs = full_spikes[predicted_labels == 0]
        self.spikes_indices = valid_spikes_locs
        self.plot_detected_spikes()
    ...
    def save_pandas(self):
        ...
        except FileNotFoundError as e: # TODO FIX ME
            # This error will raise for example when a channel name has a / in it
            print(e)

    def create_pandas_df(self):
        print('Creating pandas dataframes dataentry.py line 201')
        page = self.controller.get_page("ParameterEntryPage")
        btw_brst_tol = float(page.analysis_parameters['Burst Tolerance (defaults at .25s)'].get())
        ...
        notebook = [page.analysis_parameters['Notebook (Optional)'].get()] * len(time_of_spike)
        ...
        stim_kind = [page.analysis_parameters['Stimulation Kind (Optional)'].get()] * len(time_of_spike)
        stim = [page.analysis_parameters['Stimulation (Optional)'].get()] * len(time_of_spike)
        ...
        # TODO: READD ABOVE, disabled for proctolin analysis 8-28-22
    ...
    def spikes_to_bursts(self):
        ...
        try:
            ...
            print(self.spikes_df['condition'])
